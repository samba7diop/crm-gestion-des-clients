...
@section('content')
    <div class="max-w-3xl">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-slate-900">Nouveau devis</h1>
                <p class="text-sm text-slate-500">Créez un devis commercial pour un contact.</p>
            </div>
            <a href="{{ route('quotes.index') }}" class="rounded-md bg-slate-100 px-4 py-2 text-sm text-slate-700 hover:bg-slate-200">Retour</a>
        </div>

        <form action="{{ route('quotes.store') }}" method="POST" class="space-y-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            @csrf
            ...
            <div>
                ...
                <textarea name="lignes" rows="5" class="mt-2 w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-900" placeholder='[{"description":"Produit A","quantite":1,"prix":100}]'>{{ old('lignes') }}</textarea>
            ...
            <div class="grid gap-6 sm:grid-cols-3">
                <div>
                    ...
                    <input type="number" step="0.01" name="total_ht" value="{{ old('total_ht', 0) }}" class="mt-2 w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-900" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">TVA (%)</label>
                    <input type="number" step="0.01" name="tva" value="{{ old('tva', 20) }}" class="mt-2 w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-900" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Total TTC</label>
                    <input type="number" step="0.01" name="total_ttc" value="{{ old('total_ttc', 0) }}" class="mt-2 w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-900" required>
                </div>
            </div>
            ...
            <div class="grid gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="date" name="date_validite" value="{{ old('date_validite') }}" class="mt-2 w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-900" required>
                </div>
            </div>
